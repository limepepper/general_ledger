...
from general_ledger.django.models import Bank, Payment
...
class TestPaymentSearching:
    ...
    def test_search_payments(self):
        ...
        qs = Payment.objects.filter(
            Q(
                items__from_object_id__in=bank_1.bankstatementline_set.values_list(
                    "id", flat=True
                )
            )
            | Q(
                items__to_object_id__in=bank_2.bankstatementline_set.values_list(
                    "id", flat=True
                )
            )
        )
